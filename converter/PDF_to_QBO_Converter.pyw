...
PURCHASE_URL = "https://checkout.dodopayments.com/buy/pdt_0NnCoI0xvHnpbtDdxYxvE?quantity=1"
...
class ConverterApp:
    ...
    def create_qbo(self):
        ...
        # REPORT USAGE
        #
        # Counts against the shared conversion quota for this
        # license_key (not this machine) -- reported only after
        # the QBO file has actually been written to disk, so a
        # cancelled save dialog or write failure above doesn't
        # burn a conversion.
        # ----------------------------------------------------
        ...
        try:
            usage = self.license_manager.report_conversion()

            remaining = usage.get("quota_remaining")

            if usage.get("allowed") is False:
                messagebox.showwarning(
                    "Conversion Limit Reached",
                    "This license has used all of its conversions "
                    "for the current billing period.\n\n"
                    "The file above was still created and saved -- "
                    "contact TaxPreparerTools.com to add more "
                    "conversions or upgrade your plan."
                )
            elif remaining is not None:
                quota_note = (
                    f" ({remaining} conversions left this period.)"
                )

        except Exception:
            # Usage reporting is best-effort: a network hiccup here
            # should never block a file the user already has saved.
            pass
